...
**Status:** Planned; live game evidence outstanding. **Planning date:** 2026-09-23.
...
3. A narrow, read-only collection path is demonstrated feasible in Chrome/Chromium and Firefox, or documented as unsupported with evidence and a fallback decision. The proof covers XHR and fetch visibility, startup timing, change events, MAIN-to-isolated-to-runtime message validation, and required permissions. A research spike may use disposable instrumentation; no production collector is an M1 deliverable.
...
- **Exit:** At least one controlled live session maps each required fact to a source or identifies a gap; XHR/fetch and event timing are tested in Chrome/Chromium and Firefox; MAIN-world access, isolated bridge, runtime boundary, permissions, and failure behavior are documented; data minimization is reviewed.
...
2. **[#23](https://github.com/FoE-City-Optimizer/city-optimizer/issues/23), browser boundary spike** — in controlled sessions test early MAIN-world XHR/fetch visibility and change events, isolated messaging validation, minimum host permissions, and Chrome/Chromium/Firefox differences. Record why any approach fails and alternatives. No reusable production integration code.
